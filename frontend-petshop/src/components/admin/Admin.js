...
const adminFeatures = ["Products", "Customers", "Orders"];

export default function Admin() {
  const [display, setDisplay] = useState(true);
  const navigate = useNavigate();

  const { adminState, adminDispatch, productState, productDispatch } =
    ...
    <div className="mt-[1rem]">
      ...
      <div className="flex flex-col md:flex-row p-5">
        <div className="relative md:w-[1/4] p-5">
          ...
          <img
            src={adminPic}
            className=" w-[100%] h-[200px] md:w-[60%] md:h-[90%] rounded-xl shadow-xl shadow-black "
            alt="adminpic "
          />
              ...
              {adminFeatures.map((item, idx) => (
                ...
                >
                  ...
                  <div className="font-bold  flex flex-col md:flex-row  ">
                    ...
                    <p className="md:ml-5 ring ring-orange-500 rounded-full p-2 w-[50px] h-[50px] flex justify-center items-center bg-orange-500">
                      {" "}
                      {item === "Products"
                        ? product.length
                        : item === "Customers"
