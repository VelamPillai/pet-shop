...
export default function PetMenu() {
  ...
  return (
    <div>
              ...
              return (
                ...
                >
                  {menuName === "dog" || menuName === "cat"
                    ? `${menuName.slice(0, 1).toUpperCase()}${menuName.slice(
                        1
                      )} ${item}`
                    : menuName === "sale %" &&
                      `${menuName
                        .split(" ")[0]
                        .slice(0, 1)
                        .toUpperCase()}${menuName.slice(1, 4)} ${item}`}
                </li>
              );
            })}
          </ul>
      ...
      <div className="flex flex-col  sm:hidden justify-center items-center  p-5 w-full">
        <button
          type="button"
          onClick={handleBtnClick}
          className=" mt-1 p-5 bg-orange-100/75 hover:bg-orange-200 focus:ring-4 focus:outline-none focus:ring-orange-300  sm:font-medium rounded-lg text-sm   sm:px-4 py-2.5 text-center inline-flex justify-between items-center dark:bg-orange-600 "
        >
          Subcategories
          <MdKeyboardArrowDown />
        </button>
        ...
        {!hideOption && (
          <div className="z-10 p-5 bg-orange-100/75 divide-y divide-orange-100  shadow w-[200px]   ">
            {(menuName === "dog" ||
              menuName === "cat" ||
              menuName === "sale %") && (
              <ul className="p-1 space-y-1 text-xs  sm:text-sm text-orange-700  w-[100%]">
                {petMenu.map((item, idx) => {
                  return (
                    <li
                      key={idx}
                    ...
                    >
                      {menuName === "dog" || menuName === "cat"
                        ? `${menuName
                            .slice(0, 1)
                            .toUpperCase()}${menuName.slice(1)} ${item}`
                        : menuName === "sale %" &&
                          `${menuName
                            .split(" ")[0]
                            .slice(0, 1)
