...
export default function Orders() {
  ...
  user && user.ordersId.map((item)=>  adminOrder.includes(item) && userOrder.push(item) )
  ...
  const profileHandler = (e) => {
    ...
    user && (e.target.textContent === 'Account' ? navigate('/account') : e.target.textContent === 'Profile' ? navigate('/profile') :  navigate('/orders') )
  ...
  const displaySingleOrder = (orderId) => {
    ...
    fetch(`http://localhost:8000/orders/userOrders/${orderId}`, {
      method: "GET",
      headers: { token: localStorage.getItem("token") },
      ...
      .then((result) => {
        if (result.success) {
          ...
          productDispatch({ type: "setSingleOrder", payload: { data: result.data } })
  ...
  return (
    <div>
      <p></p>
      <p className='flex justify-center items-center text-[1rem] font-bold text-shadow '>Orders</p>
